...
class ViewController: UIViewController {
    ...
    private func aes_256_ecb_objective_c() {
        ...
        AESCipher.ecbCipher(contentData, key: keyData, settings: settings) { (data, error) in
            if error != AESCipherErrorNone {
                ...
                print("**** objective-c aes256 begain: ****")
                if let data = data  {
                    print("encryptedData = \(data.toHexString())")
    ...
    private func cryptoSwift() {
        do {
            let key = "passwordpasswordpasswordpassword".bytes // 32-bytes
            let aes256 = try AES(key: key,
                                 blockMode: ECB(),
                                 padding: .zeroPadding)
            let content = "TechTutorialsX!TechTutorialsX!" // 30-bytes
            /*
             let content = "TechTutorialsX!TechTutorialsX!" // 30-bytes
             encryptedData = f879168e6a3f622bf5ea1a2c2982847cc7bb70649693089c9388fddb4b37ed20
             decryptedData = 546563685475746f7269616c735821546563685475746f7269616c735821
             
             let content = "TechTutorialsX!TechTutorialsX" // 29-bytes
             encryptedData = f879168e6a3f622bf5ea1a2c2982847cd5782993df63386fc7382dbb2f267aea
             decryptedData = 546563685475746f7269616c735821546563685475746f7269616c7358
             
             let content = "TechTutorialsX!TechTutorials" // 28-bytes
             encryptedData = f879168e6a3f622bf5ea1a2c2982847ce316d69d82f31026074738c3df94d5f9
             decryptedData = 546563685475746f7269616c735821546563685475746f7269616c73
             */
            print("before encrypted count = \(content.bytes.count)")
            print("before encrypted = \(content.bytes.toHexString())")
            // encrypted
            let encryptedData = try aes256.encrypt(content.bytes)
            print("encryptedData = \(encryptedData.toHexString())")
            // decrypted
            let decryptedData = try aes256.decrypt(encryptedData)
